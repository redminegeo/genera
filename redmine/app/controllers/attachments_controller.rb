...
class AttachmentsController < ApplicationController
  ...
  before_filter :find_editable_attachments, :only => [:edit_all, :update_all]
  ...
  def edit
    signature =  detect_signature_edit
    render :action => 'edit', :locals => {:password_type => signature}
  end
...
##vc
  ...
  def destroy
    ...
    respond_to do |format|
      if @user.present?
        format.html { redirect_to_referer_or user_path(@user) }
      else
  ...
  def find_attachment
    ...
    # @fjaneta ini
    if @attachment.container_type.to_s == 'Principal'
...
##vc
  ...
  # for signature edit @fjaneta ini
  def detect_signature_edit
    res = false
    if request.original_url.to_s.rindex('/attachments/users').present? 
