...
class IssuesController < ApplicationController
  ...
  def create
    ...
    # @fjaneta
    if params[:assigned_user_ids].nil? 
      flash.now[:error] = l(:error_validate_presence_asignds)
    else
      @issue.assigned_user_ids = params[:assigned_user_ids] #Llama al Custom setter assigned_user_ids -> @fjaneta A_03
    end
    # @fjaneta
    call_hook(:controller_issues_new_before_save, { :params => params, :issue => @issue })
    @issue.save_attachments(params[:attachments] || (params[:issue] && params[:issue][:uploads]))
    if @issue.save && !params[:assigned_user_ids].nil?
      ...
      respond_to do |format|
        format.html {
          ...
          redirect_after_create
         if params[:issue][:assigned_to_id].nil?
         @issue.save_asignar= User.current.id
         else
         @issue.save_asignar= params[:issue][:assigned_to_id]
         end
          @issue.draft_issue
        }
  ...
  def destroy
    ...
    @issues.each do |issue|
      begin
        ...
        issue.deleted_user = User.current.id
        issue.save()
        # @fjaneta fin
