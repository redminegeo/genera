...
module IssuesHelper
  ...
  def issue_heading(issue)
     if issue.is_draft?
      h("#{issue.tracker} # #{issue.id} (Temporal) ")
    else
       if issue.code_num?
           h("#{issue.tracker} ##{issue.id} (#{issue.code_num})")
          else
  ...
  def render_issue_subject_with_tree(issue)
    ...
    if issue.is_draft?
      subject = content_tag('span', l(:field_is_draft), :class => 'draft') + ' ' + subject
    end

    s << content_tag('h1', subject)
    s << '</div>' * (ancestors.size + 1)
    s.html_safe
  end

  def render_descendants_tree(issue)
    s = '<form><table  bgcolor="PaleTurquoise" class="list issues">'
