...
module ApplicationHelper
  ...
  def link_to_issue(issue, options={})
    ...
    #text = options[:tracker] == false ? "##{issue.id}" : "#{issue.tracker} ##{issue.id}"
      if issue.is_draft?
       text = options[:tracker] == false ? "##{issue.id} (Temporal) " : " #{issue.tracker} ##{issue.id} (Temporal)"
      else
          if issue.code_num?
             text = options[:tracker] == false ? "##{issue.id} (#{issue.code_num})" : "#{issue.tracker} ##{issue.id} (#{issue.code_num})"
           else
              text = options[:tracker] == false ? "##{issue.id}" : "#{issue.tracker} ##{issue.id}"
           end
      end
