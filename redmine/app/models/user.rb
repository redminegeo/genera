...
class User < Principal
  ...
  #@fjaneta
  has_many :asigneds
  has_many :issues, through: :asigneds
  acts_as_attachable
  #@fjaneta
...
def update_role (rol_project)
 @roles =Role.find_by(:name => rol_project[:role].to_s)
ActiveRecord::Base.connection.execute("select * from fn_perfil_proyecto("+User.current.id.to_s+","+@roles.id.to_s+","+rol_project[:project].to_s+")")
end
