...
#  before_update :asigner_rol
  belongs_to :asigner_rol
  belongs_to :project
  belongs_to :tracker
  ...
  #
  after_create :save_assigneds
  after_update :update_assigneds
  after_update :draft_issue
#vcolcha
after_save :update_num
  has_many :assigneds,lambda { where(:is_deleted => false) }, :dependent => :destroy
  has_many :users, through: :assigneds

  def assigned_user_ids=(value)
    @assigned_user_ids = value
  end
  #@fjaneta 
  ...
  def self.visible_condition(user, options={})
    Project.allowed_to_condition(user, :view_issues, options) do |role, user|
      if user.id && user.logged?
        ...
        when 'own'
          user_ids = [user.id] + user.groups.map(&:id).compact
          "(#{table_name}.author_id = #{user.id} OR #{table_name}.assigned_to_id IN (#{user_ids.join(',')})  OR #{table_name}.is_private = #{connection.quoted_false} )"
  ...
  def visible?(usr=nil)
    (usr || User.current).allowed_to?(:view_issues, self.project) do |role, user|
      if user.logged?
        ...
        when 'own'
          self.author == user || user.is_or_belongs_to?(assigned_to) || !self.is_private?  
        else
          false
        end
      else
...
##vc  def attributes_editable?(user=User.current)
##vc    user.allowed_to?(:edit_issues, project)
##vc end
...
##vc
  ...
  #end
  def asigner_rol
   result = ActiveRecord::Base.connection.execute("select project_id from members mb, member_roles mbr where mb.id = mbr.member_id and mbr.role_id =5 AND mb.user_id ="+User.current.id.to_s)
   numero= result[0]["project_id"]
   self.project = (project_id.present? ? Project.find_by_id(numero) : nil)
  end
  ...
  def project_id=(project_id)
   if project_id.to_s != self.project_id.to_s
   #result = ActiveRecord::Base.connection.execute("select project_id from members mb, member_roles mbr where mb.id = mbr.member_id and mbr.role_id =5 AND mb.user_id ="+User.current.id.to_s)
   #idproject= result[0]["project_id"]
      #self.project = (project_id.present? ? Project.find_by_id(idproject) : nil)
       self.project = (project_id.present? ? Project.find_by_id(project_id) : nil)
    end
    self.project_id
  end
  ...
  safe_attributes 'is_draft',
    :if => lambda {|issue, user| user.allowed_to?(:set_issues_private, issue.project) ||
        (issue.author_id == user.id && user.allowed_to?(:set_own_issues_private, issue.project))
    }
  ...
  def assigneds_all
    assigneds = ""
    if self.assigneds.present?
      self.assigneds.each do |assigned|
        if assigned.user.present?
          assigneds = assigneds +", "+ assigned.user.to_s
        end
        if assigned.group.present?
          assigneds = assigneds +", "+ assigned.group.to_s
        end
      end
    end

    if self.assigned_to.present?
      assigneds = self.assigned_to.to_s
    end
    assigneds = assigneds.sub(',', '')
    assigneds
  end

def save_asignar=(values)
if  self.is_draft == true
if values.to_s.length == 0
ActiveRecord::Base.connection.execute("UPDATE issues SET  assigned_to_id="+ User.current.id.to_s+" where id="+self.id.to_s+";")
else
ActiveRecord::Base.connection.execute("UPDATE issues SET  assigned_to_id="+values.to_s+" where id="+self.id.to_s+";")
end
end
end
...
def draft_issue
if  self.is_draft  == true
ActiveRecord::Base.connection.execute("UPDATE issues SET status_id=4  where id="+self.id.to_s+";")
ActiveRecord::Base.connection.execute("DELETE FROM custom_values where id=(SELECT cvkey.id from custom_values cvkey, issues iss, custom_fields cf where cf.id=custom_field_id and cvkey.customized_id = iss.id and  cf.field_format = 'serial_number' and iss.id ="+self.id.to_s+");")
end
end
...
def update_num
ActiveRecord::Base.connection.execute("UPDATE issues SET code_num=(SELECT UPPER(cvkey.value) from custom_values cvkey, issues iss, custom_fields cf where cf.id=custom_field_id and cvkey.customized_id = iss.id and  cf.field_format = 'serial_number' and iss.id ="+self.id.to_s+")where id="+self.id.to_s+";")
end
#vcolcha
def delete_num
if  self.status_id == 4
...
def update_status
if  self.is_draft  == true
ActiveRecord::Base.connection.execute("UPDATE issues SET status_id=1 , is_draft=false, author_id="+User.current.id.to_s+"where id="+self.id.to_s+";")
ActiveRecord::Base.connection.execute("UPDATE issues SET code_num=(SELECT UPPER(cvkey.value) from custom_values cvkey, issues iss, custom_fields cf where cf.id=custom_field_id and cvkey.customized_id = iss.id and  cf.field_format = 'serial_number' and iss.id ="+self.id.to_s+")where id="+self.id.to_s+";")
  ...
  def save_assigneds
    if @assigned_user_ids.nil?
      ...
      @assigned_user_ids.each do |user_id|
        ActiveRecord::Base.connection.execute("INSERT INTO assigneds(user_id, issue_id,created_user,created_at) VALUES ("+user_id.to_s+", "+self.id.to_s+","+User.current.id.to_s+",current_timestamp);")  
      end
    end  
  end

  def update_assigneds # a espera de optiimizacion? @fjaneta
    if @assigned_user_ids.present?
      assigneds = Assigned.where("issue_id = ?",self.id)
      if assigneds.present?

        @assigned_user_ids.each do |user_id|
          _found = false
          assigneds.each do |assigned|
            if assigned.user_id.to_s == user_id.to_s #habilitar los assigneds removidos anteriormente
              _found = true
              ActiveRecord::Base.connection.execute("UPDATE assigneds SET is_deleted = false,updated_at=current_timestamp,updated_user="+User.current.id.to_s+" WHERE id="+assigned.id.to_s+";")
            end
          end
          if !_found #Create
            ActiveRecord::Base.connection.execute("INSERT INTO assigneds(user_id, issue_id,created_user,created_at) VALUES ("+user_id.to_s+", "+self.id.to_s+","+User.current.id.to_s+",current_timestamp);")  
        ...
        assigneds.each do |assigned|
          _found = false
          @assigned_user_ids.each do |user_id|
            if assigned.user_id.to_s == user_id.to_s
              _found = true
            end
          end
          if !_found
            ActiveRecord::Base.connection.execute("UPDATE assigneds SET deleted_user="+User.current.id.to_s+",deleted_at=current_timestamp,is_deleted = true WHERE id="+assigned.id.to_s+";")
          end
        end
