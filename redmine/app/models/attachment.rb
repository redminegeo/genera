...
class Attachment < ActiveRecord::Base
  ...
  class Result 
    ...
    def initialize(_status, _message)
      @status = _status
      @message = _message
    end

    def to_s
      return 'Result [ status: '+status.to_s+', message: '+message.to_s+']'
    end
  end
  ...
  def validate
    begin
      ...
      result = Result.new(true,'DEFAULT')
      path = File.join(PATH_SIGNATURE_BASE,self.disk_directory,self.disk_filename)
      if File.exist? path
        pkcs = nil
        Thread.new { pkcs = OpenSSL::PKCS12.new(File.read(path),Redmine::Ciphering.decrypt_text(self.description)) }.join
        cert = OpenSSL::X509::Certificate.new(pkcs.certificate.to_pem)
        cn = cert.subject.to_a.find { |name, _, _| name == 'CN' }[1]
        serial = cert.serial
        
        if pkcs.ca_certs.present?
          pkcs.ca_certs.each do |c|
            _cn = c.subject.to_a.find { |name, _, _| name == 'CN' }[1]
            if (cn.eql? _cn) && (c.serial.to_i > serial.to_i)
              c.inspect
              cert = OpenSSL::X509::Certificate.new(c.to_pem)
            end
          end
        end

        if Time.now < cert.not_before
          result.status = false
          result.message = 'La fecha de vigencia de la Firma Electrónica aun no ha iniciado, Inicia el '+cert.not_before.to_s
        elsif Time.now > cert.not_after
          result.status = false
          result.message = 'La fecha de vigencia de la Firma Electrónica ha caducado el '+cert.not_after.to_s
        else
          result.message = 'Fecha de vigencia: '+cert.not_before.to_s+' hasta '+cert.not_after.to_s
        end
      else
        result.status = false
        result.message = 'La Firma Electrónica no se encuentra en la ruta '+path.to_s
      end
    rescue Exception => e  
      log.error(e.message)
      result.status = false
      result.message = 'La contraseña de la Firma Electrónica es incorrecta'
    end  
    return result
  end
  # @fjaneta fin
